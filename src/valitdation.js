...
function checkMarket (market) {
    if(!(market.includes(alphaCodes))) return "Market must be an ISO 3166-1 alpha-2 country code.";
}
...
function checkGenres (genre) {
    if(!(genre.includes(availableGenres))) return "Genre must be on of the available genres on Spotify.";
}
